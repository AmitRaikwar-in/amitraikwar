...
const LightPillar: React.FC<LightPillarProps> = ({
  ...
  useEffect(() => {
    ...
    const fragmentShader = `
      ...
      void main() {
        ...
        for(int i = 0; i < ITERATIONS; i++) {
          ...
          // Naturally fade out the accumulation for rays that miss the pillar, 
          // preventing infinite background haze and keeping the background transparent.
          float hazeFade = smoothstep(10.0, 2.0, radialBound);
          color += (gradient / fieldDistance) * hazeFade;
        ...
        color = max(vec3(0.0), color); // Prevent negative colors
        
        // Output proper alpha so the background is truly transparent
        float alpha = clamp(max(color.r, max(color.g, color.b)) * uIntensity, 0.0, 1.0);
    ...
    const pillarRotRad = (propsRef.current.pillarRotation * Math.PI) / 180.0;
